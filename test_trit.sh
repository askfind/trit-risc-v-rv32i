...
testentry() {
    echo "Debug: Testing file '$1' with params: t='$2' d='$3' expected='$4'"

    # Проверяем существование эмулятора
    if [ ! -f "./emu" ]; then
        echo "Error: emu not found in current directory"
        return 1
    fi

    # Пробуем выполнить команду с выводом отладки
    echo "Running: ./emu -m -t \"$2\" -d \"$3\" \"$1\""
    res=$(./emu -t "$2" -m -d "$3" "$1" 2>&1)
    #echo "Command exit code: $?"
    #echo "Output length: ${#res} characters"
