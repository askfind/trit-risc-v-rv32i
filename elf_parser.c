    ...
    for(int i=0; i<Ehdr->e_shnum;i++){
        ...
        if (strcmp(".text", name) == 0) {   // ROM
            ...
            for(int j=0;j<Shdr[i].sh_size;j+=2){
				...
				uint8_to_tr8(&c->inst_rom[j], obj[j]);
				uint8_to_tr8(&c->inst_rom[j+1],obj[j+1]);
			}	
            log_printf("\n");
        }
            ...
            for(int j=0;j<Shdr[i].sh_size;j+=2){
                ...
                log_printf("RAM: %04X %02X%02X\n", data_ram_offset + j, obj[j], obj[j+1]);
               	...
               	uint8_to_tr8(&c->data_ram[data_ram_offset+ j], obj[j]);                
				uint8_to_tr8(&c->data_ram[data_ram_offset+ j+1], obj[j+1]);
            }
            log_printf("\n");
        }
    }
